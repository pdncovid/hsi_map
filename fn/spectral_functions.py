...
def get_laplacian(mat, normalize=True, degree_int=True, symmetry=True, view=True):
...
def get_eigen_gap(mat, sig, eigengap_only=False, view=True):
    [eig_, _] = np.linalg.eig(mat)
    # eig_ = (eig_ - np.amin(eig_)) / (np.amax(eig_) - np.amin(eig_))
    eigen_gap = np.abs(np.diff(eig_))
    if view:
        plt.figure(figsize=(12, 4))
        plt.subplot(121), plt.stem(eig_), plt.xlim([-1, 10]), plt.ylim([0, 1])
